...
export const ScheduleForm = ({ storeId }: { storeId: string }) => {
  ...
  useEffect(() => {
    ...
    //     const existingSchedule = schedules.filter((schedule) => schedule.date = date.getTime())[0];
    //     if (!existingSchedule) {
  ...
  return isScheduleLoading || submittingSchedule ? (
    ...
    <div className="sm:max-w-screen-lg m-auto pt-3">
      ...
      <div className="w-full text-right text-xs italic text-red-accent-400 h-5 pt-1">
        {scheduleChanged
          ? "Unsaved changes. Click Save Schedule above when you're done."
          : ""}
      </div>
                ...
                <ScheduleItem key={schedule.date} storeScheduleItem={schedule}>
                  <div className="absolute top-1 right-3 flex items-center">
                    ...
                    >
                      <Button
                        variant="link"
                        className="flex items-center gap-2 text-sm text-red-accent-700"
                      >
